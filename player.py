...
class Player:
    ...
    def player_coll(self, rect_x, rect_y, new_x, new_y):
        # Trouver la distance entre le joueur et les bords du mur
        ...
        pg.draw.line(self.game.screen, (255, 0, 0), (closest_x, closest_y), (new_x, new_y))

        # Vérifier si la distance est inférieure à zéro
        if dist <= 0:
            return True
        else:
            return False
    ...
    def draw_player(self):
        ...
        if self.game.en_mouvement:
            ...
            player_image = self.game.images[self.image_direction][0]
        player_rect = pg.Rect(int(self.x), int(self.y), 48, 96)
        self.game.screen.blit(player_image, player_rect)
        pg.draw.rect(self.game.screen, (0, 0, 0), (int(self.x), int(self.y), 48, 96), 1)
    ...
    def animate(self, dt):
        ...
        if self.game.courir:
            self.game.image_index = int((self.animation_time / ANIMATION_SPEED) % 3) + 2
        else:
            self.game.image_index = int((self.animation_time / ANIMATION_SPEED) % 2) + 1
